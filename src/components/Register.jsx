  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setData({ name: "", userType: "", email: "", password: "", mobile: "" }); // Clear form
      } else {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-full">
      ...
      <div className="w-full md:w-1/2 bg-green-900 flex flex-col items-center justify-center px-6 py-10">
        ...
        <form className="w-full max-w-xs" onSubmit={onSubmitHandler}>
          ...
          <input name="password" onChange={onChangeHandler} value={data.password} type="password" placeholder="Password"
            className="w-full p-3 border rounded-md mb-4 text-gray-700 bg-white" />
          <input name="mobile" onChange={onChangeHandler} value={data.mobile} type="tel" placeholder="Mobile"
            className="w-full p-3 border rounded-md mb-4 text-gray-700 bg-white" />

          <button type="submit" className="w-full bg-yellow-400 text-black font-semibold py-3 rounded-full hover:bg-yellow-500 transition">
            Verify & Register
          </button>
        </form>
      ...
      <div className="w-full md:w-1/2 h-full hidden md:flex overflow-hidden">
        <img src={pagi2} alt="Side Image" className="w-full h-full object-cover" />
      </div>
    </div>
  );
}
